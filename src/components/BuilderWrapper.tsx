...
import { MoveUp, MoveDown, Trash, Settings, Layers, Plus, Edit3, Image as ImageIcon, ArrowLeft, ArrowRight, GripVertical } from 'lucide-react';
...
export function BuilderWrapper({ children, id, index, isFirst, isLast, isBuilder, arrayKey = 'order', style = {}, className = '' }: any) {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    ...
    const isContentElement = target.closest('.builder-media-hoverable') ||
                             target.closest('.builder-text-hoverable') ||
                             target.closest('img') ||
    ...
    if (!isFigmaDrag && isContentElement && !target.closest('.builder-drag-handle')) {
      return;
    }
       ...
       <AnimatePresence>
         ...
         {isBuilder && isHovered && !dragSize && !contextMenu && (
           ...
           <motion.div 
             ...
             onMouseDown={(e) => {
               const target = e.target as HTMLElement;
               if (target.closest('.builder-drag-handle')) {
                 handleMouseDown(e);
               } else {
                 e.stopPropagation();
               }
